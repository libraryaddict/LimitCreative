...
public class InteractionListener implements Listener {
   ...
   public InteractionListener(JavaPlugin plugin) {
      ...
      DisabledItemsMessage = ChatColor.translateAlternateColorCodes('&', getConfig().getString("DisabledItemsMessage"));
   ...
   public void onBlockPlace(BlockPlaceEvent event) {
      ...
      if (MarkBlocks && (isCreativeItem(event.getItemInHand()) || event.getPlayer().getGameMode() == GameMode.CREATIVE)) {
         ...
         if (event.getEntity() instanceof Player && ((Player) event.getEntity()).getGameMode() != GameMode.CREATIVE) {
            ...
            for (int i = 0; i < 4; i++) {
               ...
               if (isCreativeItem(item)) {
                  ...
                  final HashMap<Integer, ItemStack> leftovers = ((Player) event.getEntity()).getInventory().addItem(
                        item);
